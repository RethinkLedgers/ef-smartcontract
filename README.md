...
In the `ui/src/UseKnownParties.tsx` file you should have COMMENTED OUT the following parts:
...
and UNCOMMENTED the following parts:
...
In the `ui/src/UseKnownParties.tsx` file you should have UNCOMMENTED and COMMENTED OUT the appropriate parts similarly to how it is described above for running th eapp locally, just the other way round.
...
Unfortunatley specifying different party identifiers and display names is not possible yet on Daml Hub, so until this feature becomes available also on Daml Hub, we use the workaround, described above. 
